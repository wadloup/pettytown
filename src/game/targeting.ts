...
export const canTargetNpc = (state: GameState, npcId: string) => {
  ...
  if (!npc || npc.zoneId !== state.currentZoneId) return false;
...
export const canTargetLocation = (state: GameState, locationId: string) => {
  ...
  return state.locations.some((location) => location.id === locationId && location.zoneId === state.currentZoneId);
};
...
export const targetableNpcIds = (state: GameState) =>
  state.npcs.filter((npc) => npc.zoneId === state.currentZoneId && canTargetNpc(state, npc.id)).map((npc) => npc.id);

export const targetableLocationIds = (state: GameState) =>
  state.locations.filter((location) => location.zoneId === state.currentZoneId && canTargetLocation(state, location.id)).map((location) => location.id);
